...
import { R6_STORY, R6_CORRECT_ANSWER, R6_MISSION_IMAGE, MISSIONS } from '../../constants';
...
export default function R6RetentionGame({ onComplete, onBack, startTime }: Props) {
  ...
  const handleSubmit = () => {
    ...
    if (normalized === R6_CORRECT_ANSWER) {
      setCleared(true);
      setError('');
    } else {
      setError('ACCESS DENIED - 키워드가 아닙니다.');
    }
  };
  ...
  return (
    <div className="nb-card rounded-2xl p-6 max-w-2xl mx-auto">
      ...
      {!cleared ? (
        <>
          <p className="text-cl-text/70 text-sm mb-4 leading-relaxed">{R6_STORY}</p>
          ...
          {showImage && (
            <div className="mb-4">
              <button onClick={() => setPopupImage(true)} className="w-full">
                <img src={R6_MISSION_IMAGE} alt="리텐션 미션" className="w-full rounded-xl border border-cl-navy/20 cursor-pointer hover:opacity-90" />
              </button>
              <p className="text-[10px] text-cl-text/30 text-center mt-1">이미지를 터치하면 크게 볼 수 있습니다</p>
            </div>
          )}
          {popupImage && (
            <ImagePopup src={R6_MISSION_IMAGE} alt="리텐션 미션" onClose={() => setPopupImage(false)} />
          )}
